...
Заголовки/тексты: напоминание — «Скоро: <eventName>» / «До цели <N> дн., откладывайте
<monthlyNeeded>». Событие — «<title>» / «<дата>, <тип>» либо «сегодня». `data` несёт
`{type, id}` для будущей навигации.
