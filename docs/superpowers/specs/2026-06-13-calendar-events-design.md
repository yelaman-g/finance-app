...
- **Экран** (`CalendarPage`): `TableCalendar` (месячная сетка) с `eventLoader`,
  ...
  время/«весь день», тип-иконка); FAB → форма создания. Тап по событию → правка/удаление.
...
- **Форма** (bottom sheet или страница): название, описание, дата (date picker),
  ...
  interval + опц. until); поле «бюджет» + «уведомлять за дн.» — **только при
  freq=NONE**. Редактирование/удаление серии.
